...
CONFIG="${CONFIG:-/work/postgresql-config.yaml}"
...
if [[ -f "${CONFIG}" ]]; then
    ...
    if [[ -n "${HOSTS:-}" ]]; then
        ...
        echo "  -v ./postgresql-config.yaml:/work/postgresql-config.yaml" >&2
...
# --- Run postgresql.py from /work/results so output lands there ---
cd /work/results

exec python3 /work/postgresql.py \
    -c "${CONFIG}" \
    --virtctl-only \
    "$@"
