...
def deploy_scripts(config: PostgreSQLTestConfig, executor: CommandExecutor) -> None:
    ...
    with ThreadPoolExecutor(max_workers=len(config.db_hosts)) as pool:
        futures = []
        for host in config.db_hosts:
            cmd = f"cd '{config.hammerdb_path}' && export GIT_SSL_NO_VERIFY=true && git clone '{config.hammerdb_repo}' ."
    ...
    with ThreadPoolExecutor(max_workers=len(config.db_hosts)) as pool:
        futures = []
        for host in config.db_hosts:
            cmd = f"chmod +x '{config.hammerdb_path}/templates/postgresql/Hammerdb-postgres-install-script'"
    ...
    for user_count in config.user_count:
        ...
        with ThreadPoolExecutor(max_workers=len(config.db_hosts)) as pool:
            ...
            for host in config.db_hosts:
                ...
                cmd = f"cd {config.hammerdb_dir} && nohup ./hammerdbcli auto runtest{vm_number}_pg.tcl > '{output_file}' 2>&1 &"
