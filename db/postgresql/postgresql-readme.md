...
`postgresql.py` is a comprehensive Python script for running PostgreSQL performance tests using HammerDB TPCC benchmarks on Kubernetes virtual machines or bare metal/KVM hosts. The script automates the entire testing workflow including package installation, database setup, test execution, VM migration testing, and results collection.
...
- Configure SSH keys using:
  ...
  ```
- Verify access works: `ssh root@your-host` should connect without password prompt
...
database:
  ...
  # host_labels: "app=postgresql"  # Method 3: Label selector (virtctl only)
...
2. **Pattern Expansion**: Bash-style ranges like `pg{1..200}` or `server{1..10}` (works for both modes)
3. **Label Selector**: Kubernetes labels like `app=postgresql-test` (virtctl mode only)
...
python3 postgresql.py --ssh-only -c postgresql-ssh-config.yaml

# Prepare hosts via SSH
python3 postgresql.py --ssh-only --prepare-hosts -c postgresql-ssh-config.yaml

# Copy results via SSH
python3 postgresql.py --ssh-only --copy-results -c postgresql-ssh-config.yaml
```

#### Virtctl Mode (OpenShift/Kubernetes VMs)
```bash
# Force virtctl mode (default behavior)
python3 postgresql.py --virtctl-only -c postgresql-config.yaml
...
- Check if output file exists: `ssh root@server1 "ls -lh /usr/local/HammerDB/test_postgresql_pg_*.out"`
...
- Check HammerDB output files on hosts manually:
  - virtctl mode: `virtctl ssh root@vmi/vm1 -n default "ls -lh /usr/local/HammerDB/test_postgresql_pg_*.out"`
  - SSH-only mode: `ssh root@server1 "ls -lh /usr/local/HammerDB/test_postgresql_pg_*.out"`
- Use `--copy-results` to re-copy if files exist

#### 6. "templates/postgresql directory not found"
**Error**: `ERROR: Clone completed but templates/postgresql directory not found`
...
1. Archived into `postgresql-results.tar.gz` on each host
