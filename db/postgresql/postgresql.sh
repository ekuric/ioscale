...
get_db_hosts() {
    ...
    # Method 1: Host pattern expansion (e.g., pg{1..200})
    ...
    if [[ "$host_pattern" != "null" && -n "$host_pattern" ]]; then
        ...
        # Use bash expansion for patterns like pg{1..200}
    ...
    if [[ "$host_labels" != "null" && -n "$host_labels" ]]; then
        ...
        if [[ "$DRY_RUN" == "false" ]] && command -v oc &> /dev/null; then
            ...
            echo "example-pg1 example-pg2"  # Placeholder for dry-run
            return 0
        fi
    fi
...
# Safely manage PostgreSQL service (with service existence check)
manage_postgresql_service() {
    local host="$1"
    local action="$2"
    local description="${3:-PostgreSQL service management}"
    ...
    case "$action" in
        ...
        "stop")
            ...
            ;;
        *)
            log_error "Invalid action '$action' for PostgreSQL service management"
            return 1
            ;;
    esac
}
...
deploy_scripts() {
    ...
    # Step 1: Clean old scripts and prepare directories in parallel
    local bg_pids=()
    for host in $DB_HOSTS; do
        execute_ssh_background "$host" \
    ...
    for host in $DB_HOSTS; do
        execute_ssh_background "$host" \
            "cd '$HAMMERDB_PATH' && export GIT_SSL_NO_VERIFY=true && git clone '$HAMMERDB_REPO' ." \
...
install_postgresql() {
    ...
    wait_for_background_jobs "PostgreSQL installation" "${bg_pids[@]}"
}
...
build_database() {
    ...
    # Step 1: Restart PostgreSQL services in parallel
    log_info "Step 1/5: Restarting PostgreSQL services on all hosts..."
    local bg_pids=()
    for host in $DB_HOSTS; do
    ...
    wait_for_background_jobs "PostgreSQL service restart" "${bg_pids[@]}"
    
    # Step 2: Wait for services to be ready
    log_info "Step 2/5: Waiting for PostgreSQL services to be ready..."
    sleep 15
...
collect_results() {
    local results_dir="${1:-./postgresql-results-$(date +%Y%m%d-%H%M%S)}"
    
    log_info "Collecting PostgreSQL test results..."
    mkdir -p "$results_dir"
    ...
    for host in $DB_HOSTS; do
        ...
        if [[ "$DRY_RUN" == "true" ]]; then
            ...
            if virtctl -n "$NAMESPACE" scp "root@vmi/$host:/usr/local/HammerDB/postgresql-results.tar.gz" "$host_dir/postgresql-results.tar.gz" 2>/dev/null; then
                log_info "Successfully copied results from $host using virtctl scp"
                
                # Extract results locally for easier access
                if command -v tar &> /dev/null; then
                    cd "$host_dir"
                    if tar -xzf postgresql-results.tar.gz 2>/dev/null; then
                        log_info "Extracted results for $host"
                        # Remove the tar file to save space, keep extracted files
                        rm -f postgresql-results.tar.gz
                    else
                ...
                if virtctl -n "$NAMESPACE" ssh -t "-o StrictHostKeyChecking=no" \
                   "root@vmi/$host" -c "cat /usr/local/HammerDB/postgresql-results.tar.gz" > "$host_dir/postgresql-results.tar.gz" 2>/dev/null; then
                    ...
                    log_info "Results are still available on $host at /usr/local/HammerDB/postgresql-results.tar.gz"
                    log_info "Manual copy command: virtctl -n $NAMESPACE ssh root@vmi/$host -c 'cat /usr/local/HammerDB/postgresql-results.tar.gz' > $host_dir/postgresql-results.tar.gz"
                fi
            fi
        fi
    done
    ...
    if [[ "$DRY_RUN" == "false" ]]; then
        ...
        log_info "PostgreSQL Test Results Summary:"
...
stop_postgresql() {
    log_info "Stopping PostgreSQL instances on all hosts..."
    
    # Step 1: Stop PostgreSQL services in parallel
    local bg_pids=()
    for host in $DB_HOSTS; do
        execute_ssh_background "$host" \
            "if systemctl list-unit-files | grep -q '^postgresql.*\.service'; then
    ...
    wait_for_background_jobs "PostgreSQL service stop" "${bg_pids[@]}"
    ...
    if [[ "$MOUNT_POINT" != "none" && "$MOUNT_POINT" != "null" ]]; then
        ...
        for host in $DB_HOSTS; do
            execute_ssh_background "$host" \
                "# Check if mount point is mounted and unmount it
                 ...
                 cd /usr/local/HammerDB && rm -f postgresql-results.tar.gz 2>/dev/null || true" \
        ...
        for host in $DB_HOSTS; do
            execute_ssh_background "$host" \
                "# PostgreSQL installation script mounts disk devices to /perf1
...
main() {
    ...
    local final_results_dir="./postgresql-results-$results_timestamp"
    
    collect_results "$final_results_dir"
    stop_postgresql
    
    log_info "PostgreSQL performance testing completed successfully"
