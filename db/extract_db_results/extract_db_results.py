...
def _pct_ratio(ratio):
    """Format ratio as percentage change vs baseline."""
...
def _test_type_stats(test_results):
    ...
    nopms = [r['nopm'] for r in test_results if r.get('nopm') is not None]
    sorted_tpms = sorted(tpms)
    n = len(tpms)
    p10 = sorted_tpms[max(0, int(n * 0.1) - 1)] if n else 0
    p90 = sorted_tpms[min(n - 1, int(n * 0.9) - 1)] if n else 0
    nopm_ratio = (statistics.mean(nopms) / statistics.mean(tpms) * 100) if tpms and nopms else None
    return {
        'n': n,
        'total_tpm': sum(tpms),
        'total_nopm': sum(nopms) if nopms else 0,
        'avg_tpm': statistics.mean(tpms) if tpms else 0,
        'avg_nopm': statistics.mean(nopms) if nopms else 0,
        'med_tpm': statistics.median(tpms) if tpms else 0,
        'min_tpm': min(tpms) if tpms else 0,
        'max_tpm': max(tpms) if tpms else 0,
        'p10': p10,
        'p90': p90,
        'stdev_tpm': statistics.stdev(tpms) if n > 1 else 0,
        'nopm_pct': nopm_ratio,
        'rows': test_results,
    }


def perf_report_filename(db_type, vm_count):
    """Build perf report filename, e.g. mariadb-perf-242machines.md."""
    return f'{db_type}-perf-{vm_count}machines.md'


def write_perf_report(all_results, db_type, output_dir, run_label=None, input_dir=None):
    """
    Write {db_type}-perf-{N}machines.md scaling/concurrency report to output_dir.

    Analyzes TPM/NOPM across test types (e.g. 1_user, 32_users) and documents how
    increasing concurrent users per VM affects cluster and per-VM throughput.
    """
    if not all_results:
        print('No results for performance report; skipping')
        return None
    ...
    if len(test_types) < 2:
        print('Performance report needs at least two test types (concurrency levels); skipping')
        return None

    by_type = {}
    for tt in test_types:
        rows = [r for r in all_results if r['test_type'] == tt]
        by_type[tt] = _test_type_stats(rows)
    ...
    baseline = by_type[baseline_tt]
    vm_count = baseline['n']

    title = f'{db_display} TPC-C performance — scaling / concurrency analysis'
    if run_label:
        title += f' ({run_label})'
    ...
    lines = [
        f'# {title}',
        '',
        'Auto-generated by `extract_db_results.py` from benchmark output under the input directory.',
        '',
    ]
    if run_label:
    ...
    lines.extend([
        f'**VMs:** {vm_count}  ',
        f'**Concurrency levels tested:** {", ".join(str(get_user_count_from_test_type(tt)) for tt in test_types)} users per VM  ',
        '',
        '| Metric | Meaning |',
        '|--------|---------|',
        '| **TPM** | Transactions per minute (all transaction types) |',
        '| **NOPM** | New-Order transactions per minute (primary TPC-C throughput metric) |',
        '',
    ])

    if all(by_type[tt]['nopm_pct'] for tt in test_types):
        avg_nopm_pct = statistics.mean(by_type[tt]['nopm_pct'] for tt in test_types)
        lines.append(
            f'NOPM is **~{avg_nopm_pct:.1f}% of TPM** across concurrency levels — transaction mix is stable.'
        )
        lines.append('')
    ...
    lines.extend([
        '## Executive summary',
        '',
        '| Users / VM | Avg TPM / VM | Cluster TPM | Cluster NOPM | vs baseline cluster |',
        '|-----------:|-------------:|------------:|-------------:|--------------------:|',
    ])
    for tt in test_types:
        s = by_type[tt]
        users = get_user_count_from_test_type(tt)
        vs = s['total_tpm'] / baseline['total_tpm'] * 100 if baseline['total_tpm'] else 0
        lines.append(
            f'| **{users}** | {_fmt_int(s["avg_tpm"])} | **{_fmt_millions(s["total_tpm"])}** | '
            f'**{_fmt_millions(s["total_nopm"])}** | {vs:.0f}% |'
        )
    lines.append('')
    ...
    for i in range(1, len(test_types)):
        ...
        vm_mult = cur_u / prev_u if prev_u else 1
        tpm_ratio = by_type[cur_tt]['total_tpm'] / by_type[prev_tt]['total_tpm']
        avg_ratio = by_type[cur_tt]['avg_tpm'] / by_type[prev_tt]['avg_tpm']
        lines.append(
            f'- **{prev_u} → {cur_u} users/VM** ({vm_mult:.0f}× clients): cluster TPM '
            f'**{tpm_ratio:.3f}×** ({_pct_ratio(tpm_ratio)}), avg TPM/VM **{avg_ratio:.3f}×** '
            f'({_pct_ratio(avg_ratio)})'
        )
    lines.append('')

    last_tt = test_types[-1]
    last_ratio = by_type[last_tt]['total_tpm'] / baseline['total_tpm']
    if last_ratio >= 0.9:
        lines.append(
            f'- Cluster throughput remains **≥{last_ratio*100:.0f}%** of baseline at highest concurrency '
            f'— moderate OLTP scaling.'
        )
    elif last_ratio >= 0.7:
        lines.append(
            f'- **Saturation between mid and high concurrency** — cluster TPM at highest load is '
            f'**{last_ratio*100:.0f}%** of baseline.'
        )
    else:
        lines.append(
            f'- **Heavy saturation at high concurrency** — cluster TPM falls to **{last_ratio*100:.0f}%** '
            f'of baseline ({_pct_ratio(last_ratio)}).'
        )
    lines.append('')

    # Per-user efficiency
    lines.extend(['## TPM per concurrent user (avg per VM)', '', '| Users / VM | TPM / user |', '|-----------:|-----------:|'])
    for tt in test_types:
        users = get_user_count_from_test_type(tt)
        per_user = by_type[tt]['avg_tpm'] / users if users else 0
        lines.append(f'| {users} | {_fmt_int(per_user)} |')
    lines.append('')
    ...
    for tt in test_types:
        ...
        users = get_user_count_from_test_type(tt)
        lines.append(
            f'| {users} | {_fmt_int(s["avg_tpm"])} | {_fmt_int(s["med_tpm"])} | '
            f'{_fmt_int(s["min_tpm"])} | {_fmt_int(s["max_tpm"])} | '
            f'{_fmt_int(s["p10"])} – {_fmt_int(s["p90"])} | {_fmt_int(s["stdev_tpm"])} |'
        )
    lines.append('')

    # Step-by-step sections
    lines.append('## Step-by-step concurrency impact')
    lines.append('')
    for i in range(1, len(test_types)):
        prev_tt, cur_tt = test_types[i - 1], test_types[i]
        prev_u = get_user_count_from_test_type(prev_tt)
        cur_u = get_user_count_from_test_type(cur_tt)
        prev_s, cur_s = by_type[prev_tt], by_type[cur_tt]
        tpm_ratio = cur_s['total_tpm'] / prev_s['total_tpm']
        avg_ratio = cur_s['avg_tpm'] / prev_s['avg_tpm']
        nopm_ratio = cur_s['total_nopm'] / prev_s['total_nopm'] if prev_s['total_nopm'] else None
        lines.extend([
            f'### {prev_u} → {cur_u} users per VM',
            '',
            '| | Change |',
            '|---|--------|',
            f'| Cluster TPM | **{tpm_ratio:.3f}×** ({_pct_ratio(tpm_ratio)}) |',
            f'| Avg TPM / VM | **{avg_ratio:.3f}×** ({_pct_ratio(avg_ratio)}) |',
        ])
        if nopm_ratio:
            lines.append(f'| Cluster NOPM | **{nopm_ratio:.3f}×** ({_pct_ratio(nopm_ratio)}) |')
        lines.append(f'| Client load / VM | **{cur_u / prev_u:.0f}×** |')
        lines.append('')

    # 1-user bimodal (only if baseline is lowest user count and spread is wide)
    if baseline_tt == test_types[0]:
        high = [r for r in baseline['rows'] if r['tpm'] >= 90000]
        mid = [r for r in baseline['rows'] if 50000 <= r['tpm'] < 90000]
        low = [r for r in baseline['rows'] if r['tpm'] < 50000]
        if high and mid and baseline['stdev_tpm'] > baseline['avg_tpm'] * 0.2:
            lines.extend([
                '## Baseline VM spread (lowest concurrency)',
                '',
                f'At **{get_user_count_from_test_type(baseline_tt)} user/VM**, TPM varies '
                f'{_fmt_int(baseline["min_tpm"])} – {_fmt_int(baseline["max_tpm"])} (σ ≈ {_fmt_int(baseline["stdev_tpm"])}):',
                '',
                f'- **≥90k TPM:** {len(high)} VMs (avg {_fmt_int(statistics.mean(r["tpm"] for r in high))})',
                f'- **50–90k TPM:** {len(mid)} VMs (avg {_fmt_int(statistics.mean(r["tpm"] for r in mid))})',
                f'- **<50k TPM:** {len(low)} VMs',
                '',
                'Investigate node/storage placement or warm-up for stragglers.',
                '',
            ])

    # Per-VM  baseline → highest degradation
    if len(test_types) >= 2:
        high_tt = test_types[-1]
        ratios = []
        by_vm = defaultdict(dict)
        for r in all_results:
            u = get_user_count_from_test_type(r['test_type'])
            by_vm[r['vm_name']][u] = r['tpm']
        base_u = get_user_count_from_test_type(baseline_tt)
        high_u = get_user_count_from_test_type(high_tt)
        for vm, d in by_vm.items():
            if base_u in d and high_u in d and d[base_u] > 0:
                ratios.append(d[high_u] / d[base_u])
        if ratios:
            lines.extend([
                f'## Per-VM retention ({high_u} vs {base_u} users/VM)',
                '',
                f'| Stat | TPM ratio ({high_u} ÷ {base_u}) |',
                '|------|------------------------------:|',
            ...
            high_base = [
                (vm, d[base_u], d[high_u], d[high_u] / d[base_u])
                for vm, d in by_vm.items()
                if base_u in d and high_u in d and d[base_u] >= 90000
            ]
            if high_base:
                high_base.sort(key=lambda x: x[3])
                lines.extend([
                    f'### High baseline VMs (≥90k TPM @ {base_u} user) at {high_u} users',
                    '',
                    '| VM | Baseline TPM | High-concurrency TPM | Retained |',
                    '|----|-------------:|---------------------:|---------:|',
                ])
                for vm, b, h, r in high_base[:8]:
                    lines.append(f'| {vm} | {_fmt_int(b)} | {_fmt_int(h)} | {r:.0%} |')
                lines.append('')

    # Worst/best at each concurrency level
    for tt in test_types:
        users = get_user_count_from_test_type(tt)
        sorted_rows = sorted(by_type[tt]['rows'], key=lambda r: r['tpm'])
        lines.extend([
            f'## Best / worst VMs @ {users} users/VM',
            '',
            '| Rank | VM | TPM |',
            '|------|-----|----:|',
        ])
        for r in sorted_rows[:5]:
            lines.append(f'| Worst | {r["vm_name"]} | {_fmt_int(r["tpm"])} |')
        for r in reversed(sorted_rows[-5:]):
            lines.append(f'| Best | {r["vm_name"]} | {_fmt_int(r["tpm"])} |')
        lines.append('')
    ...
    lines.extend([
        '## Planning notes',
        '',
        '| Goal | Suggested users/VM | Notes |',
        '|------|-------------------:|-------|',
    ])
    best_tt = max(test_types, key=lambda tt: by_type[tt]['total_tpm'])
    best_u = get_user_count_from_test_type(best_tt)
    lines.append(
        f'| Peak cluster TPM | {best_u} | {_fmt_millions(by_type[best_tt]["total_tpm"])} cluster TPM |'
    )
    lines.append(
        f'| Per-VM sizing @ highest concurrency | {get_user_count_from_test_type(test_types[-1])} | '
        f'plan ~{_fmt_int(by_type[test_types[-1]]["avg_tpm"])} TPM/VM '
        f'(~{_fmt_int(by_type[test_types[-1]]["avg_nopm"])} NOPM) |'
    )
    lines.append('')
    ...
    lines.extend([
        '## Source files',
        '',
        f'- `{db_type}_detailed_results.csv`',
        f'- `{db_type}_overall_summary.csv`',
    ])
    for tt in test_types:
        lines.append(f'- `{db_type}_summary_{tt}.csv`')
    lines.append('')
...
def write_compare_perf_index(output_dir, perf_report_entries):
    ...
    lines = [
        '# Performance reports — compare mode index',
        '',
        'Per-run scaling reports (`*-perf-*machines.md`) for each `--input-dir` / `--run-label`:',
        '',
        '| Run label | Report | Input directory |',
        '|-----------|--------|-----------------|',
    ]
    for entry in perf_report_entries:
        label = entry['label']
        report = entry.get('report_path')
        input_dir = entry.get('input_dir', '')
        if report:
            rel = os.path.relpath(report, output_dir)
            lines.append(f'| {label} | [`{os.path.basename(report)}`]({rel}) | `{input_dir}` |')
        else:
            lines.append(f'| {label} | *(not generated — need 2+ concurrency levels)* | `{input_dir}` |')
    lines.append('')

    # Cross-run cluster TPM table from each run's overall_summary.csv
    all_test_types = set()
    run_summaries = []
    for entry in perf_report_entries:
        dir_output = entry['dir_output']
        db_type = entry.get('db_type', 'database')
        overall_csv = os.path.join(dir_output, f'{db_type}_overall_summary.csv')
        by_type = {}
        if os.path.isfile(overall_csv):
            with open(overall_csv, newline='') as f:
                for row in csv.DictReader(f):
                    tt = row.get('Test_Type', '')
                    if tt:
                        by_type[tt] = {
                            'total_tpm': int(float(row['Total_TPM'])),
                            'avg_tpm': float(row['Avg_TPM']),
                            'vms': int(row['VM_Count']),
                        }
                        all_test_types.add(tt)
        run_summaries.append({'label': entry['label'], 'by_type': by_type})

    if all_test_types:
        sorted_types = sorted(all_test_types, key=get_user_count_from_test_type)
        lines.extend([
            '## Cluster TPM across runs',
            '',
            '| Run label | ' + ' | '.join(
                f'{get_user_count_from_test_type(tt)} users/VM' for tt in sorted_types
            ) + ' |',
            '|-----------|' + '|'.join(['---:'] * len(sorted_types)) + '|',
        ])
        for rs in run_summaries:
            cells = []
            for tt in sorted_types:
                stats = rs['by_type'].get(tt)
                cells.append(_fmt_millions(stats['total_tpm']) if stats else '—')
            lines.append(f'| {rs["label"]} | ' + ' | '.join(cells) + ' |')
        lines.append('')

        lines.extend([
            '## Avg TPM / VM across runs',
            '',
            '| Run label | ' + ' | '.join(
                f'{get_user_count_from_test_type(tt)} users/VM' for tt in sorted_types
            ) + ' |',
        ...
        for rs in run_summaries:
            ...
            for tt in sorted_types:
                stats = rs['by_type'].get(tt)
                cells.append(_fmt_int(stats['avg_tpm']) if stats else '—')
            lines.append(f'| {rs["label"]} | ' + ' | '.join(cells) + ' |')
        lines.append('')
...
def process_postgresql_results(input_dir, output_dir, chart_type='scatter', user_filter=None,
                               show_values=False, run_label=None):
    """
    ...
    Args:
        ...
        chart_type (str): Type of chart to create ('scatter', 'line', 'bar')
        user_filter (set): Optional set of test types to include (e.g., {"1_user", "20_users"})
        run_label (str): Optional label for perf report title (compare mode / --run-labels)

    Returns:
        str | None: Path to {db_type}-perf-{N}machines.md if written, else None
    """
    ...
    report_path = write_perf_report(
        all_results, db_type, output_dir, run_label=run_label, input_dir=input_dir)
...
def main():
    ...
    if len(valid_dirs) == 1:
        ...
        process_postgresql_results(
            valid_dirs[0], output_dir, args.chart_type, user_filter, args.show_values,
            run_label=run_labels[0] if use_custom_labels else None,
        )
        ...
        for input_dir, run_label in zip(valid_dirs, run_labels):
            ...
            report_path = process_postgresql_results(
                input_dir, dir_output, args.chart_type, user_filter, args.show_values,
                run_label=run_label,
            )
            db_type = 'database'
            if report_path:
                m = re.match(r'^(.+)-perf-\d+machines\.md$', os.path.basename(report_path))
                if m:
                    db_type = m.group(1)
            elif os.path.isdir(dir_output):
                for name in os.listdir(dir_output):
                    if name.endswith('_overall_summary.csv'):
                        db_type = name.replace('_overall_summary.csv', '')
                        break
            perf_report_entries.append({
                'label': run_label,
                'input_dir': input_dir,
