# mssqldb.py Container (Linux MSSQL Server)

Run HammerDB TPCC tests against MSSQL Server on **Linux** VMs from a container.
This is the Linux-based MSSQL test (not the Windows `mssqlwin.py` variant).
...
|---|---|---|
| `CONFIG` | `/work/mssql-config.yaml` | Path to the YAML config file inside the container |
| `HOSTS` | -- | Space-separated host names (pick one of HOSTS / HOST_PATTERN / HOST_LABELS) |
| `HOST_PATTERN` | -- | Brace-expansion pattern, e.g. `mssql-{1..10}` |
| `HOST_LABELS` | -- | OCP label selector for VMs |
| `NAMESPACE` | `default` | OpenShift namespace |
| `DESCRIPTION` | -- | Test run description |
| `DISK_LIST` | `/dev/vdc` | Block device for MSSQL data |
...
after the image name are forwarded to `mssqldb.py`.
...
running** to measure the impact of VM migration on MSSQL Server performance.
...
period**. HammerDB has a rampup phase before the timed test begins:
...
## Difference from mssqlwin.py

This container runs `mssqldb.py` which tests **MSSQL Server on Linux VMs**.
The other container (`mssqlwin.py` / `Dockerfile`) tests **MSSQL Server on
Windows VMs** with PowerShell scripts and Windows-specific template files.

| | mssqldb.py (this) | mssqlwin.py |
|---|---|---|
