...
def _extract_mssql_tpm(text: str) -> Optional[int]:
    """Extract SQL Server TPM from HammerDB TEST RESULT line (both output formats)."""
    match = re.search(r'System achieved (\d+) SQL Server TPM at \d+ NOPM', text)
    if match:
        return int(match.group(1))
    match = re.search(r'System achieved \d+ NOPM from (\d+) SQL Server TPM', text)
    if match:
        return int(match.group(1))
    return None


def _find_mssql_templates_dir(bundled_path: str) -> str:
    """Locate templates/mssql inside a bundled HammerDB scripts tree."""
    candidates = [
        os.path.join(bundled_path, "templates", "mssql"),
        os.path.join(bundled_path, "db", "templates", "mssql"),
        os.path.join(bundled_path, "scripts", "templates", "mssql"),
        os.path.join(bundled_path, "scripts", "mssql"),
    ]
    for candidate in candidates:
        install_script = os.path.join(candidate, "Hammerdb-mssql-install-script")
        if os.path.isfile(install_script):
            return candidate
    searched = ", ".join(candidates)
    raise FileNotFoundError(
        f"MSSQL templates not found under {bundled_path}. Checked: {searched}"
    )


def _create_mssql_templates_archive(bundled_path: str) -> str:
    """Create a tar.gz archive with templates/mssql/ layout for remote extraction."""
    templates_dir = _find_mssql_templates_dir(bundled_path)
    tmp = tempfile.NamedTemporaryFile(prefix="mssql-templates-", suffix=".tar.gz", delete=False)
    tmp.close()
    with tarfile.open(tmp.name, "w:gz") as tar:
        for entry in os.listdir(templates_dir):
            tar.add(
                os.path.join(templates_dir, entry),
                arcname=os.path.join("templates", "mssql", entry),
            )
    return tmp.name
...
def validate_inputs(config: MSSQLTestConfig) -> None:
    ...
    if not config.dry_run and config.hammerdb_source == 'bundled':
        try:
            _find_mssql_templates_dir(config.hammerdb_bundled_path)
...
def ensure_packages_installed(config: MSSQLTestConfig, executor: CommandExecutor) -> None:
    ...
    package_list = "curl vim wget iproute" if config.hammerdb_source == 'bundled' else "curl vim wget git iproute"
    ...
    with ThreadPoolExecutor(max_workers=config.max_pool_workers(len(config.db_hosts))) as pool:
        ...
        for host in config.db_hosts:
            cmd = (
                ...
                f"  dnf -y --nobest install {package_list}; "
...
def _set_mssql_script_permissions(config: MSSQLTestConfig, executor: CommandExecutor) -> None:
    """Set execute permissions on the MSSQL install script on all hosts."""
...
def _deploy_bundled_scripts_to_host(
    ...
    """Copy bundled MSSQL templates archive to a single host and extract it."""
    remote_archive = "/tmp/mssql-templates.tar.gz"
    hammerdb_path_escaped = config.hammerdb_path.replace("'", "'\"'\"'")
    ...
    try:
        ...
        install_script_escaped = (
            f"{config.hammerdb_path}/templates/mssql/Hammerdb-mssql-install-script"
        ).replace("'", "'\"'\"'")
        extract_cmd = (
            f"bash -c '"
...
def _deploy_bundled_scripts(config: MSSQLTestConfig, executor: CommandExecutor) -> None:
    """Deploy MSSQL templates from the container image to all hosts."""
    if config.dry_run:
        logger.info(
    ...
    logger.info(
        f"Deploying bundled MSSQL templates from {config.hammerdb_bundled_path} "
        f"to {config.hammerdb_path} on all hosts..."
    )
    archive_path = _create_mssql_templates_archive(config.hammerdb_bundled_path)
    try:
        with ThreadPoolExecutor(max_workers=config.max_pool_workers(len(config.db_hosts))) as pool:
            futures = {
...
def _deploy_remote_git_scripts(config: MSSQLTestConfig, executor: CommandExecutor) -> None:
    """Deploy MSSQL templates by cloning a git repository on each host (legacy mode)."""
    ...
    with ThreadPoolExecutor(max_workers=config.max_pool_workers(len(config.db_hosts))) as pool:
        ...
        for host in config.db_hosts:
            ...
            cmd = (
                ...
                f"    mkdir -p templates && ln -sf ../scripts/templates/mssql templates/mssql; "
                f"  elif [ -d \"scripts/mssql\" ]; then "
                f"    mkdir -p templates && ln -sf ../scripts/mssql templates/mssql; "
                f"  else "
                f"    echo \"ERROR: templates/mssql directory not found\"; "
                f"    exit 1; "
                f"  fi; "
                f"  test -d \"templates/mssql\"; "
                f"else "
                f"  echo \"ERROR: Git clone failed\"; "
                f"  exit 1; "
                f"fi"
                f"'"
            )
...
def deploy_scripts(config: MSSQLTestConfig, executor: CommandExecutor) -> None:
    ...
    if not config.dry_run:
        _set_mssql_script_permissions(config, executor)
...
def configure_mssql_memory(config: MSSQLTestConfig, executor: CommandExecutor) -> None:
    """Set max server memory on all hosts via sp_configure (no restart required)."""
    ...
    logger.info(f"Setting max server memory to {cap_mb} MB on all hosts...")
    ...
    with ThreadPoolExecutor(max_workers=config.max_pool_workers(len(config.db_hosts))) as pool:
        futures = []
        for host in config.db_hosts:
            cmd = f"sqlcmd -S localhost -U sa -P {password} -C -Q \"{sql}\" 2>&1"
            futures.append(
                pool.submit(
    ...
    logger.info(f"Max server memory set to {cap_mb} MB on all hosts")
...
def _build_process_check_cmd(vm_number: str) -> str:
    """Return ps pipeline that matches only this host's HammerDB build script."""
    return f"ps aux | grep -F 'build{vm_number}_mssql.tcl' | grep -v grep | wc -l"


def _build_output_verify_cmd(hammerdb_dir: str, output_file: str) -> str:
    """Remote command to inspect HammerDB MSSQL build log for completion/errors."""
    return (
        f"cd {hammerdb_dir} && "
...
def _poll_build_host_status(
    ...
    *,
    ...
    """
    Poll one host during MSSQL build monitoring.
    ...
    check_cmd = _build_process_check_cmd(vm_number)
    success, output = executor.execute_command(
        host,
        check_cmd,
...
def _finalize_build_hosts(
    executor: 'CommandExecutor',
    config: MSSQLTestConfig,
    host_build_info: Dict[str, Tuple[str, str]],
    hosts: List[str],
) -> List[str]:
    ...
    with ThreadPoolExecutor(max_workers=config.max_pool_workers(len(config.db_hosts))) as pool:
        ...
        for future, vm_number, host, output_file in futures:
            ...
            if not success:
                ...
                verify_cmd = _build_process_check_cmd(vm_number)
                    ...
                    if test_file.startswith("test_mssql_") and test_file.endswith(".out"):
                        ...
                        try:
                            with open(test_file_path, 'r', encoding='utf-8', errors='ignore') as f:
                                content = f.read()
                                tpm = _extract_mssql_tpm(content)
                                if tpm is not None:
...
def prepare_hosts(config: MSSQLTestConfig, executor: CommandExecutor) -> None:
    ...
    if config.dry_run:
        ...
        if config.max_server_memory_mb is not None:
            logger.info(f"  4. Set max server memory to {config.max_server_memory_mb} MB")
    ...
    configure_mssql_memory(config, executor)
...
WORKFLOW:
    ...
    parser.add_argument('--prepare-hosts', action='store_true',
                       help='Only run preparation steps (install packages, deploy scripts, MSSQL Server setup)')
    ...
    if config.dry_run:
        ...
        if config.copy_results:
            ...
            steps = [
                ...
                "Deploy HammerDB scripts",
                "Install MSSQL Server",
            ]
            if config.max_server_memory_mb is not None:
                steps.append(f"Set max server memory to {config.max_server_memory_mb} MB")
            steps.extend([
                "Build TPCC database",
                "Run performance tests",
                "Collect test results from all VMs",
                "Stop MSSQL Server instances and cleanup storage",
            ])
