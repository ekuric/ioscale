...
`mariadb.py` is a comprehensive Python script for running MariaDB performance tests using HammerDB TPCC benchmarks on Kubernetes virtual machines. The script automates the entire testing workflow including package installation, database setup, test execution, VM migration testing, and results collection.
...
database:
  ...
  # host_labels: "app=mariadb"    # Method 3: Label selector
...
2. **Pattern Expansion**: Bash-style ranges like `db{1..200}` or `mariadb-{001..050}`
3. **Label Selector**: Kubernetes labels like `app=mariadb-test` or `env=prod,tier=db`
...
1. Archived into `mariadb-results.tar.gz` on each VM
