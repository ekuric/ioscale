...
OPTIONS:
    ...
    --prepare-hosts     Only run preparation steps (install packages, git clone, MariaDB setup)
...
EXAMPLES:
    ...
    $0 --prepare-hosts          # Only prepare hosts (install packages, MariaDB)
...
WORKFLOW:
    ...
    1. Preparation: ./mariadb.sh --prepare-hosts    # Install packages, MariaDB
    2. Testing:     ./mariadb.sh                   # Run performance tests
...
prepare_hosts() {
    ...
    log_info "This will install packages, clone repositories, and setup MariaDB"
...
main() {
    if [[ "$PREPARE_HOSTS" == "true" ]]; then
        log_info "Starting MariaDB HammerDB TPCC testing script (PREPARATION MODE)"
    else
        log_info "Starting MariaDB HammerDB TPCC testing script (FULL TEST MODE)"
    fi
