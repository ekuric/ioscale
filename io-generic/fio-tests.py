...
class CommandExecutor:
    ...
    def check_task_running(self, host: str, task_pattern: str = "fio.*testfile") -> bool:
        ...
        success, output = self.execute_command(host, cmd, f"Checking if process '{task_pattern}' is running", max_retries=1, retry_interval=1, timeout=30)
...
def main():
    ...
    linux_hosts = [h for h in config.vm_hosts if h not in config.windows_hosts]
    windows_hosts = [h for h in config.vm_hosts if h in config.windows_hosts]
        ...
        with ThreadPoolExecutor(max_workers=len(windows_hosts)) as pool:
            ...
            for future in as_completed(futures):
                ...
                if not success:
                    ...
                    # DEBUG: List contents of root_dir to verify what was copied
                    # This was added for debugging FIO installation issues on Windows.
                    # Uncomment below if you need to verify directory contents during troubleshooting.
                    # list_cmd = f"powershell -Command \"Get-ChildItem -Path '{root_dir_ps_with_slash}' -ErrorAction SilentlyContinue | Select-Object Name, PSIsContainer, Length, LastWriteTime, FullName | Format-Table -AutoSize | Out-String\""
                    # list_success, list_output = executor.execute_command(host, list_cmd, "Listing directory contents", timeout=10)
                    # if list_success and list_output:
                    #     logger.info(f"Contents of {root_dir_ps_with_slash} on {host}:")
                    #     # Log each line with indentation for readability
                    #     for line in list_output.strip().split('\n'):
                    #         if line.strip():
                    #             logger.info(f"  {line}")
                    # else:
                    #     logger.warning(f"Failed to list directory contents on {host}")
                    #     if list_output:
                    #         logger.warning(f"List command output: {list_output.strip()}")
                    # 
                    # # DEBUG: Also check if fio is a directory or file, and list its contents if it's a directory
                    # # This was added for debugging FIO installation issues on Windows.
                    # # Uncomment below if you need to verify fio item type and contents during troubleshooting.
                    # check_fio_cmd = f"powershell -Command \"$item = Get-Item '{root_dir_ps_with_slash}fio' -ErrorAction SilentlyContinue; if ($item) {{ if ($item.PSIsContainer) {{ Write-Host 'IS_DIRECTORY'; Get-ChildItem -Path '{root_dir_ps_with_slash}fio' -ErrorAction SilentlyContinue | Select-Object Name | Format-Table -AutoSize | Out-String }} else {{ Write-Host 'IS_FILE' }} }} else {{ Write-Host 'NOT_FOUND' }}\""
                    # check_fio_success, check_fio_output = executor.execute_command(host, check_fio_cmd, "Checking fio item type and contents", timeout=10)
                    # if check_fio_success and check_fio_output:
                    #     if 'IS_DIRECTORY' in check_fio_output:
                    #         logger.info(f"fio is a directory on {host}. Contents:")
                    #         # Extract and log the directory listing
                    #         lines = check_fio_output.split('\n')
                    #         in_listing = False
                    #         for line in lines:
                    #             if 'IS_DIRECTORY' in line:
                    #                 in_listing = True
                    #                 continue
                    #             if in_listing and line.strip():
                    #                 logger.info(f"  {line}")
                    #     elif 'IS_FILE' in check_fio_output:
                    #         logger.warning(f"fio is a FILE (not directory) on {host} - this is unexpected!")
                    #     elif 'NOT_FOUND' in check_fio_output:
                    #         logger.warning(f"fio item not found on {host} at {root_dir_ps_with_slash}fio")
                    
                    # DEBUG: Also try listing with dir command to match what user sees
                    # This was added for debugging FIO installation issues on Windows.
                    # Uncomment below if you need to verify directory contents during troubleshooting.
                    # dir_cmd = f"powershell -Command \"cd {root_dir_ps_with_slash}; dir | Out-String\""
                    # dir_success, dir_output = executor.execute_command(host, dir_cmd, "Listing with dir command", timeout=10)
                    # if dir_success and dir_output:
                    #     logger.info(f"Directory listing (dir command) for {root_dir_ps_with_slash} on {host}:")
...
def prepare_storage(config: FioTestConfig, executor: CommandExecutor) -> None:
    ...
    logger.info("Step 1/5: Validating test devices on all hosts...")
    ...
    logger.info("Step 2/5: Unmounting existing mounts on Linux hosts...")
    if linux_hosts:
        with ThreadPoolExecutor(max_workers=len(linux_hosts)) as pool:
            futures = []
            for host in linux_hosts:
    ...
    if windows_hosts:
        logger.info("Step 3/5 (Windows): Preparing storage on Windows hosts using provision-data-disk.ps1...")
    ...
    logger.info("Step 4/5: Creating test directories on all hosts...")
    with ThreadPoolExecutor(max_workers=len(config.vm_hosts)) as pool:
        futures = []
        for host in linux_hosts:
    ...
    # Step 4: Format devices (Linux only - Windows handled by provision script)
    if linux_hosts:
        logger.info("Step 4/5: Formatting devices on Linux hosts (WARNING: destructive operation)...")
        with ThreadPoolExecutor(max_workers=len(linux_hosts)) as pool:
            futures = []
            for host in linux_hosts:
                device = config.storage_devices[host]
